import { getCsrfTOkenN0G4nGBoYY } from "../modules/getCsrfToken"
import { postarFormulario } from "../modules/postarFormulario"
...
export class cadastroValidator {
    ...
    // LIMPANDO OS CAMPOS
        limpandoDados() {
            const meuArray = []
            meuArray.push(this.nomeCompleto.value, this.email.value, this.senha.value, this.confirmSenha.value)

            for (let i = 0; i < meuArray.length; i++) {  // Garantindo que tudo seja uma String
            ...
            if (nomeUser.length == 0 || typeof nomeUser == 'undefined') {    // Se nome tiver vazio, retorne erro para usuário
            ...
            if (emailUser.length == 0 || typeof emailUser == 'undefined') {    // Se e-mail tiver vazio, retorne erro para usuário
                this.erros.push('Campo e-mail não pode estar vazio')
                this.mostrandoErros(this.email)
                return
            } else this.limpandoErros(this.email)

            if (senhaUser.length == 0 || typeof senhaUser == 'undefined') {    // Se senha tiver vazio, retorne erro para usuário
                this.erros.push('Campo senha não pode estar vazio')
                this.mostrandoErros(this.senha)
                return
            } else this.limpandoErros(this.senha)

            if (confirmSenhaUser.length == 0 || typeof confirmSenhaUser == 'undefined') {    // Se confirmSenha tiver vazio, retorne erro para usuário
...
// VALIDANDO SENHA NO FORMULÁRIO DE CADASTRO
    async validandoSenha() {
        ...
        const csrfToken = await this.obtendoCsrfToken()
        this.enviarFormulario(csrfToken)
        
    }

    async obtendoCsrfToken() {
        try {
            const csrfToken = await getCsrfTOkenN0G4nGBoYY()
            const inputCsrfToken = document.querySelector('.inputCsrfToken')
...
//Envio do formulário
    async enviarFormulario (csrfToken) {
        const inputCsrfToken = document.querySelector('.inputCsrfToken')
        inputCsrfToken.value = csrfToken
        if (inputCsrfToken.value == csrfToken) {
            const nome_completo = this.nomeCompleto.value
            ...
            await postarFormulario(formDataUser)
            .then(console.log('postei e sai correndo'))
